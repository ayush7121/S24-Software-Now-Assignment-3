...
JUMP_HEIGHT = -15
MAX_LIVES = 3
MAX_HEALTH_PER_LIFE = 3
OBSTACLES_PER_LEVEL = 25
INITIAL_SPEED = -3
ENEMY_HEALTH = 3
DIAMOND_POINTS = 100
...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.HEIGHT = 95
        try:
            original_image = pygame.image.load(os.path.join('assets', 'hero.png')).convert_alpha()
            self.image = pygame.transform.scale(original_image, (self.WIDTH, self.HEIGHT))
        except Exception as e:
            print(f"Hero image error: {e}. Using placeholder.")
    ...
    def heal(self):
        self.health = min(self.health + 1, MAX_HEALTH_PER_LIFE)
...
class Projectile(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.HEIGHT = 40
        try:
            original_image = pygame.image.load(os.path.join('assets', 'fire.png')).convert_alpha()
            self.image = pygame.transform.scale(original_image, (self.WIDTH, self.HEIGHT))
        except Exception as e:
            print(f"Fire image error: {e}. Using placeholder.")
            self.image = pygame.Surface((self.WIDTH, self.HEIGHT))
            self.image.fill(RED)
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, x, speed):
        ...
        self.HEIGHT = 95
        try:
            original_image = pygame.image.load(os.path.join('assets', 'enemy.png')).convert_alpha()
            self.image = pygame.transform.scale(original_image, (self.WIDTH, self.HEIGHT))
        except Exception as e:
...
class Diamond(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.HEIGHT = 30
        try:
            original_image = pygame.image.load(os.path.join('assets', 'diamond.png')).convert_alpha()
            self.image = pygame.transform.scale(original_image, (self.WIDTH, self.HEIGHT))
        except Exception as e:
            print(f"Diamond image error: {e}. Using placeholder.")
            self.image = pygame.Surface((self.WIDTH, self.HEIGHT))
            self.image.fill((0, 255, 255))  # Cyan color for diamond placeholder
